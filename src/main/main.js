...
// Function to create the app menu
...
// Check license before launching the app
async function checkLicense() {
  // Initialize license manager if it doesn't exist
  if (!licenseManager) {
  ...
  console.log('[FAKE LICENSE] Simulated valid license, starting app...');
...
app.whenReady().then(() => {
  // Set the icon for the whole app
  if (process.platform === 'win32') {
    app.setAppUserModelId(app.getName());
  }

  // Check license before launching app
  checkLicense();

  app.on('activate', () => {
    if (BrowserWindow.getAllWindows().length === 0) {
      // Check license again if no windows are open
      checkLicense();
    }
  });
});
...
function setupIPCHandlers() {
    ...
    if (mainViewWindow && mainViewWindow.views) {
      mainViewWindow.views.forEach(view => {
        if (view.webContents && !view.webContents.isDestroyed()) {
          ...
          console.log(`Config sent to view ${view.viewNumber} (${view.viewType}): bitrate=${bitrate}, region=${settings.region}`);
        }
      });
    }
  });
  ...
  // Events for synchronized keyboard inputs
  ...
  // Handle scrolling of the main container
...
function startMacro() {
  ...
  // Start interval for simulating main keys
  const mainKeyInterval = setInterval(simulateMainKey, 2000); // every 2 seconds

  // Start interval for simulating random keys
